...
shifting spot shifts the landing law by the same amount. The conditional return law---and
hence the smile in moneyness---then depends on the regime $(f,s)$ alone, not on the spot level.
This single property keeps every dynamic readout, the SSR in particular, an exact closed form
(Section~\ref{sec:ssr}). It also makes propagation an exact Gaussian-mixture map
...
structure. Gy\"ongy's identity chooses it so the forward map lands on the SANOS target
...
coefficient map, and the fourth (marginal-matching) is the leverage. Calibration therefore fits
...
calibration absorb the discretisation. Because $\bar\gamma$ and the leverages are fit \emph{at} the
grid, the model hits its ATM and SSR targets at any node count, and refining merely re-shifts
$\bth$. The continuous limit $n_{\mathsf{F}},n_{\mathsf{S}}\to\infty$ is thus a structural relationship, not a practical convergence claim at these counts. The counts trade resolution against parameter
...
weight $w_l$ and the regime transitions: $\mathrm{GM}\ast\mathrm{GM}=\mathrm{GM}$, exactly.
...
martingale-lock drift (Section~\ref{sec:invariance}), and $\bar V_{f,s}$ the kernel's \emph{full} one-step increment
...
translation-invariant and exact. For \emph{marginal matching}---each step re-seeded from the target law, with regimes at their
spot-uncorrelated stationary spread---the residual is discretisation rather than bias. The
discrete Dupire under-reads the local variance at the narrowest (short-$T$) marginals
($\sim$50--85\,bp). This tightens with a central-difference Dupire, finer grids, and a
tail-clamped leverage. Under multi-step \emph{concatenation from a
...
SPX). Approximating $\sigma_{\mathrm{LV}}\approx\sigma_{\mathrm{Dupire}}$ then \emph{under-reads}
the accumulated ATM level by of order $10\%$ in vol over a multi-month chain. There the exact
projection~\eqref{eq:gyongy} is retained: $\E[\nu\mid z]$ is negligible for reproducing marginals, but becomes a genuine
path-conditional rescaling for absolute-level outputs such as forward implied vol and hedge
P\&L.
...
separate Gy\"ongy step (Remark~\ref{rem:fitproject}), not part of recompression. The default is a deterministic \emph{moment-preserving merge}: per fibre, sort by mean, split into
...
call, $-\partial_K C=1-G(K)$: one derivative above the price, one below the density. The bound
that justifies the choice lives in the \emph{spot/strike} variable $s$ (with $G$ the law of
$S_T$):
\begin{equation}
\begin{aligned}
...
difference. That equals its total variation and---since $-\partial_K C$ is the digital---the
...
\emph{is} the time-homogeneity break. De-eventing strips the lumps, so the homogeneous generator $\bth$ calibrates on the clean
windows and the event becomes a thin separate layer. Left in, a flexible surface silently absorbs
the event premium, and the term-structure interpolation can turn
...
$[T_1,T_2]$ and $[T_3,T_4]$---the \emph{clean flanks} that name the method. The marginals $\mu_1,\dots,\mu_4$ are read from the chains. On the clean flanks the kernel is
the homogeneous diffusive kernel $\cK_{12}=\cK_{34}=\cK_{\mathrm{diff}}(\bth)$, so $\bth$ is
identified \emph{where there is no event}. Time-homogeneity then fixes the middle window's diffusive counterfactual---propagate the
...
$\mu_3\ominus\mu_3^{\mathrm{diff}}$. A pair of marginals alone does not pin the middle kernel. The bridge closes it with two
ingredients held fixed: $\bth$ from the clean flanks, and a low-cumulant parametric ansatz for
$\rho_J$.
...
jump is \emph{correlated} with the volatility move. That is a joint spot--vol property, the
event-conditional analogue of the SSR. Algorithm~\ref{alg:deevent} carries it in one factor
$\psi\in[0,1]$: the fraction of the flank kernel's stochastic-vol leverage
$(\lambda_{\mathsf{F}},\lambda_{\mathsf{S}})$ that survives the release, with the local-vol skew
floor untouched. It is read not from $\mu_3$ but from the event transition, on which the realised
covariance~\eqref{eq:ssr-model} is evaluated rather than on the stationary chain. Where cliquets
are quoted, $\psi$ is read risk-neutrally from the forward-start smile across $[T_2,T_3]$;
otherwise from the realised ATM response across the release~\eqref{eq:event-delta}.
...
The static jump sizes the \emph{gamma} leg. Its variance is the second cumulant
$J=\kappa_2(\rho_J)$, an implied one-event move $\sqrt J$ that the bridge separates from the
diffusive variance and that sizes the event straddle. The higher cumulants $\kappa_3,\kappa_4$ set
the directional tilt: risk-reversals or strangles positioned at the legs of $\rho_J$. The dynamics supply the \emph{delta} leg: the spot
...
The split is honest about where the value sits. For index macro events, whose priced jump is variance with little directional
skew~\cite{zhong2026}, the hedge is dominated by the gamma leg. The event-conditional delta
correction is then small unless the leverage is non-trivial---the empirical question of
Section~\ref{sec:deevent-synth}. For single-name earnings, whose asymmetry is
...
match, the overlay a calendar lump rather than the full local-volatility surface. Interpolation and de-eventing are one homogeneity property read two ways. The semigroup composes
\emph{within} an event-free block (Section~\ref{sec:interp}); the lump $J_k$ is the
\emph{between-block} residual it cannot bridge. Relative to the established static de-eventing cited at the head of this section,
...
\Ensure the diffusive generator $\bth$, the de-evented smile $\mu_3^{\mathrm{diff}}$, the event-jump density $\rho_J$, and the event-leverage factor $\psi$
\State \textbf{Clean fit.} Calibrate $\bth$ on the event-free flanks only (Algorithm~\ref{alg:calib} restricted to $[T_1,T_2],[T_3,T_4]$); the event-spanning window is held out, so the fit cannot absorb the event.
\State \textbf{Counterfactual.} Anchor at the last clean marginal $\mu_2$ and apply a \emph{single} step of the flank kernel, time-rescaled to length $T_3-T_2$, with the event-free leverage evaluated at $T_2$ and held across the window, and \emph{without} recompression: $\mu_3^{\mathrm{diff}}=\cK_{\mathrm{diff}}(\bth)\,\mu_2$. Propagating from spot instead cold-starts the third moment, and recompressing a single high-skew step flattens it; either reads a skew far below the market's.
...
\State \textbf{Dynamics.} Parametrise the event transition as the flank kernel with its stochastic-vol leverage scaled by a surviving fraction $\psi\in[0,1]$, $(\lambda_{\mathsf F},\lambda_{\mathsf S})\mapsto\psi\,(\lambda_{\mathsf F},\lambda_{\mathsf S})$, the local-vol skew floor untouched. Estimate $\psi$ by matching the model event-conditional SSR~\eqref{eq:ssr-model} to the measured one on $[T_2,T_3]$: risk-neutrally from the forward-start smile where cliquets are quoted, otherwise from the realised $\Delta\sigma_{\mathrm{ATM}}\!\leftrightarrow\!r$ regression across the release~\eqref{eq:event-delta}.
\State \textbf{Compose.} The event-window transition is the leverage-suppressed diffusive kernel convolved with the event jump, $\cK_{[T_2,T_3]}=\cK_{\mathrm{diff}}(\bth;\psi)\ast\rho_J$: the statics live in $\rho_J$, the dynamics in $\psi$.
\State \Return $\bth,\ \mu_3^{\mathrm{diff}},\ \rho_J,\ \psi$.
\end{algorithmic}
\end{algorithm}
...
identifiable, yet \emph{provably invisible} to any static de-eventing. It is the non-redundant degree of freedom the dynamics channel adds over the static
literature. It is also what licenses reading the real-data SSR \emph{suppression}
(Section~\ref{sec:realdata}: the same curve, on its leverage-down side) as a genuine dynamics
signal rather than a static-variance artifact. A symmetric martingale variance jump, meanwhile,
...
fits reach SSR RMS $3.2\%$ and $9.2\%$ (moderate). The residual sits in the VIX vol-of-vol (both ${\approx}22\%$): the same two-factor vov shape
ceiling, more severe here than in the 2020 high-vol regime ($9\%$), and not an SSR misfit. \emph{And the
...
options. NDX statics come from its own SANOS marginals, the leverage from its realised SSR, and the
vol-of-vol from the realised variance-of-variance of its own model-free variance-swap strip. That
last is the graded readout of Section~\ref{sec:ssr}, validated in that the reconstructed SPX
$\sqrt V$ tracks the VIX index.
Across 2018--2022, with multi-start, NDX calibrates to $5$--$12\%$ SSR and $1$--$8\%$
vol-of-vol (bar the $2021$ vov shape ceiling, ${\sim}19\%$). That is weaker than SPX in
\emph{both} blocks---its SSR RMS runs roughly double SPX's in matched years, $2018$/$2020$---as
expected from the purely physical-measure realised readout (no vol-index, no risk-neutral target)
and its ${\sim}10\%$ reconstruction bias. Cross-sectionally NDX's vol-of-vol
...
The frozen shape, however, is as much a \emph{structural constraint} as an empirical match. By translation invariance (Section~\ref{sec:invariance}) the conditional smile depends on the
...
from the last clean $\mu_2$ (Alg.~\ref{alg:deevent}, step~2), taken \emph{without} recompression. A from-spot propagation would instead cold-start the third moment. It builds skew from a point
mass over many steps, each recompression shaving it, so at the short event horizon it reads skew
${\approx}{-}0.8$ against a market ${\approx}{-}2.4$. Anchoring on $\mu_2$ inherits the skew
directly. The event jump is then a clean
...
\emph{no} directional skew. Read at \emph{matched} variance---the lump set to the exact $\kappa_2$
residual~\eqref{eq:deevent-cumulant}, so the two smiles share a second moment and only shape can
differ---the residual asymmetry is a large but \emph{event-independent} put-skew deficit,
$t{\approx}8$ on events and controls alike. That common deficit is the transition-side shape error
of a single-step, instantaneous-Gy\"ongy counterfactual (\S\ref{sec:fusion}, which pins marginals
not the transition); the net event-minus-control is $t{\approx}{-}0.7$. That is ``variance, not skew''~\cite{zhong2026} on index macro, confirmed
and robust to the counterfactual construction.

\paragraph{Real-SPX de-eventing, dynamics: a switched-off event leverage.}
The signature the static literature cannot form is in the dynamics. For FOMC and nonfarm-payroll releases the event-conditional SSR---the realised
...
$t{\approx}3.7$, stable under an $|r|$-crush control. The smile drops from near local-vol toward
sticky-strike on the release. CPI shows \emph{no} suppression in that low-inflation era
($\mathrm{dSSR}{\approx}0$, $t{\approx}0.5$); it became the dominant macro surprise only in
2021--22, inside the grind regime below. Calibrated back into the kernel, the event bump reproduces this by driving the
event-leverage factor $\psi$ of Algorithm~\ref{alg:deevent} to zero, and with it the SSR's
stochastic-vol component (${\approx}{+}0.4\to{\approx}0$; Figure~\ref{fig:ssr-eta}). The release
\emph{switches off} the spot--vol leverage rather than adding skew. That is the kernel-native
degree of freedom the marginal provably cannot carry (Section~\ref{sec:deevent-synth}), and the
complement to the symmetric variance jump the static literature already prices.

\begin{figure}[htbp]
\centering
\includegraphics[width=\textwidth]{figs/ssr_eta_switchoff.pdf}
\caption{The release switches off the spot--vol leverage. \textbf{(a)}~\emph{Mechanism.} Scaling the
stochastic-vol leverage couplings by a surviving fraction $\psi\in[0,1]$, $(\lambda_{\mathsf F},\lambda_{\mathsf S})\mapsto\psi\,(\lambda_{\mathsf
F},\lambda_{\mathsf S})$ in the calibrated kernel (2017-07-03, a steep-leverage normal-regime date,
full-objective $\bth$) sweeps the one-week SSR from its clean value $1.57$ at $\psi{=}1$ down to a floor
$0.92$ at $\psi{=}0$---a fall of $0.65$, of which the flat-leverage stochastic-vol component is $0.40$. The
realised FOMC event SSR, $0.88$, sits at that $\psi{=}0$ floor. \textbf{(b)}~\emph{Empirics.} The realised
event-conditional SSR (SPX 2015--2019, $1240$ days): FOMC falls $2.18\!\to\!0.88$
($\mathrm{dSSR}={-}1.34$, $t=3.5$), payrolls partially ($-0.67$, $t=4.0$), and CPI not at all ($+0.27$,
$t={-}0.7$) in that low-inflation era. The kernel reproduces the mechanism and the magnitude of the
suppression, not the absolute level---its clean SSR runs $0.61$ below the realised clean---so the realised
levels are not mapped to a per-type $\psi$.}
\label{fig:ssr-eta}
\end{figure}
...
minimum-variance delta. The one-day hedging-P\&L variance is minimised at $R^\star$ equal to the \emph{realised} SSR,
and the SSR-consistent delta cuts it sharply below Black (Figure~\ref{fig:hedge},
Table~\ref{tab:hedge}). Across 2016--2021, $R^\star\approx1.5$--$1.8$, tracking the realised SSR
($1.3$--$1.6$); hedging at the model's structural $R{=}1.6$ reduces the realised hedging variance
by \textbf{27--47\%} versus Black. Sticky-delta ($R{=}-1$) is \emph{worse} than Black---it moves the
...
the realised SSR, so it is largest in calm/moderate markets ($\SSR\approx\tfrac32$). The reduction is out-of-sample---a fixed structural $R$, not re-fit to each hedging
window---and survives transaction costs at SPX liquidity. The vol-adjustment term raises turnover,
but by a small fraction of the variance it removes, and a smoothed model skew (in place of the raw
per-day skew used here) would cut that turnover further. \emph{Still to come:} the full study extends this across moneyness and tenor, with tail-loss
and cost-curve detail. It ablates the VIX layer, the two-timescale memory, and the SSR target. And
it runs the real-data analogues of the Section~\ref{sec:synth} checks: the Monte-Carlo SSR
regression, and the identifiability gain of Appendix~\ref{app:ident} on the real readout.
...
$d(\mu_j\cK_j,\mu_{j+1})$ is a modelling choice, not a wall: a denser kernel, $W_1$-universal too,
shrinks it toward zero. And a few knobs pin the dynamics to interpretable regimes where an
unconstrained kernel would leave them underdetermined. Admissibility is exact regardless of
the residual (Remark~\ref{rem:fitproject}: the leverage carries the statics, recompression
re-locks the martingale). What lets universality and parsimony coexist is translation-invariance. Exact marginal match,
...
de-eventing layer of Sec.~\ref{sec:deevent} at intraday resolution), and a terminal/pin refinement as $T\to0$---plus synchronized intraday NBBO data. We leave the
...
SANOS-Evolve keeps the part of the option-surface problem that SANOS already solves well:
arbitrage-free marginals, the local-volatility content. It evolves that into a
stochastic-local-volatility model by fitting a martingale transition kernel with a latent
two-timescale volatility regime. The marginals stay strictly arbitrage-free, and the
