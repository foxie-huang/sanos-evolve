...
\usepackage{bm}
\graphicspath{{./}}   % flattened repo: figures are in ./figs/
...
\begin{abstract}
How an equity-index smile \emph{moves} when the spot moves---its skew-stickiness---governs every
forward-starting and path-dependent hedge, yet is left rigidly determined by the models that
reprice today's smile exactly (local and stochastic-local volatility), or only approximated by
those that move it freely but miss the smile (stochastic volatility). SANOS-Evolve makes that
motion a free, directly calibrated target while keeping the smile itself exact and arbitrage-free,
in closed form: a static SANOS linear program fixes strictly arbitrage-free marginals, and a
martingale two-timescale kernel is fitted between them, calibrated while the marginals stay fixed
by a discrete Gy\"ongy overlay and arbitrage-free by construction (Strassen). Every object is a
...
minimum-variance benchmark from the smile alone, with no trailing realised estimate.

\smallskip
\noindent\textbf{Keywords:} stochastic-local volatility; skew-stickiness ratio; smile dynamics;
arbitrage-free calibration; martingale optimal transport; two-timescale stochastic volatility;
Gaussian-mixture model; minimum-variance hedging.

\smallskip
\noindent\textbf{JEL classification:} G13; C61.
\end{abstract}
...
\paragraph{Martingales between discrete marginals.} SANOS-Evolve is not alone in building the
transition directly between finitely many arbitrage-free marginals---but it is alone in what it
asks of that transition. Andreasen--Huge~\cite{andreasenhuge2011} interpolate the \emph{surface}
by a one-step implicit finite difference of Dupire's equation, and mixture-preserving
schemes~\cite{vandenbergmix2026} interpolate across expiries within a fixed component
family---both discrete \emph{local}-volatility constructions, static by design. The Bass
local-volatility model~\cite{conzehl2021,backhoff2020,qin2024bass} goes further, constructing the
martingale between the marginals that is \emph{closest to Brownian motion}; Markov-functional
models~\cite{huntkennedypelsser2000} map a low-dimensional driver through a calibrated functional
to the same marginals. Both fix the marginals \emph{and thereby pin the dynamics}: the spot--vol
response is a by-product, not a control, so the SSR is not free---the local-vol rigidity we set
out to escape returns. SANOS-Evolve shares their exact-marginal, discrete-time martingale
skeleton but inverts the selection principle: the Gy\"ongy leverage overlay
(Section~\ref{sec:fusion}) carries the marginals for free, so the two-timescale kernel spends its
freedom fitting the SSR term structure as an explicit target. Same skeleton, opposite objective.
...
By \emph{discrete} we mean the time-stepping---an explicit one-step martingale kernel between the
listed maturities' marginals, not a time-discretisation of a continuous SLV diffusion and not a
discrete \emph{local}-volatility interpolation of the static surface.
...
(Appendix~\ref{app:density}). Here \emph{free} means the dynamics are decoupled from the exactly-matched marginals and dense in the translation-invariant class (Prop.~\ref{prop:dyn-dense}); the calibration exercises the first two orders---the SSR and the vol-of-vol---with the vol-of-vol skew and roughness as in-class extensions. A black cross marks joint SPX--VIX calibration (both smiles as hard
...
computation in a new local volatility model. \textit{SSRN preprint} 3853085.
...
models. \textit{arXiv preprint} arXiv:2606.12717.
